  ...
  useEffect(() => {
    const fetchRates = async () => {
      try {
        ...
        // 1. Pokušaj Vercel serverless (produkcija)
        try {
        ...
        // 2. Pokušaj mini-servis (lokalni dev)
        if (!data?.rates) {
          try {
        ...
        // Izračunaj sa maržom (koristi NBS kupovni/prodajni ako su dostupni)
        ...
        const ratesData: RateData[] = config.kursevi.valute.map(currency => {
          // Ako NBS ima svoje kupovne/prodajne, koristi njih + marža
          // Inače koristi srednji + marža
          ...
          // Dodaj korisničku maržu na NBS vrednosti
          const kupovni = kupovniBase * (1 - marzaK);
          const prodajni = prodajniBase * (1 + marzaP);
